...
\|\
Issue Lane 0: symmetric general lane Issue Lane 1: symmetric general lane\
Shared resources: LSU, multiply/reduction, streams, branch/control, DIVSTEP assist\
...
| Resource | Dispatch / shared resource | Concurrent use | Notes |
|----|----|----|----|
| Scalar ALU / flags | Either symmetric lane | One scalar op per cycle | L8/L16 read-modify-write uses same path. |
| Branch/control | Either lane → shared branch resource | May pair with independent independent work work | Branch resolves in EX/MEM. |
| AGU | Either lane; LSU owns memory AGU | One architectural address launch per cycle baseline | Streams have private address state. |
| Load/store port | Either lane → shared LSU | One memory operation launched per cycle baseline | Banked SRAM may sustain more internally. |
| Multiplier/compressor cluster | Either lane → shared mul/reduce | Pipelined initiation interval normally 1 | Long latency is scoreboarded. |
| Reduction tree | Either lane → shared mul/reduce | One DOT/SAD/REDUCE launch per cycle | Shared with relevant multiply reductions. |
| DIVSTEP assist | Either lane → reused ALU/shifter path | One step per launch | No standalone divider or hidden full-divide state. |
| Stream engines Q0/Q1 | Either lane → Q0/Q1 resource | Independent engines may advance together when one instruction uses both | Per-stream order preserved. |
...
| DIVSTEP | N/A between instructions | Interrupt may occur between retired steps | No hidden divide state; software sequence resumes normally |
...
| DIVSTEP | 1-2 | 1 | Reuses lane ALU/shifter; full division is software/runtime sequence |
...
- shared_units: LSU, multiply/reduction, stream dispatch, branch/control, and DIVSTEP assist
...
# Architecture Correction (v1.2a): Symmetric Dual-Issue Lanes

This correction supersedes any earlier wording that implied fixed instruction classes for 'one symmetric lane' and 'the other symmetric lane'. Aurora uses two largely symmetric issue lanes. Ordinary scalar, packed, shuffle, address, and control instructions may issue from either lane when dependencies and resources permit.

- Pairing legality is determined by data dependencies, ordering rules, and physical resource availability—not by a fixed slot assignment.

- Two independent ALU-class operations may issue together.

- Byte/word swap, PACK, UNPACK, ZIP, UNZIP, MOV, and similar register-layout operations are preferred pairing candidates and may execute in either lane.

- At most one branch/control-flow instruction issues per cycle.

- Memory operations are limited by the number of implemented load/store launch ports rather than issue-lane identity.

- DOT, SAD, REDUCE, multiply, divide, stream, and other specialized operations may issue from either lane but contend for their shared execution resources.

- Compiler scheduling models resource usage instead of fixed slot letters.
